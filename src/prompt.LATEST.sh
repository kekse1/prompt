...
ps1Prompt()
{
	...
	#
	#[[ $_SPACE -eq 0 ]] || write ' '
	#startBG 100 140 160 #startBG 95 160 205
	#startFG 0 0 0
	#startBG 0 0 0
	local _pwd="`pwd`"
	...
	if [[ -n "$_SLASH" ]]; then
		local white='\[\033[39m\]'
		local black='\[\033[38;2;0;0;0m\]'
		#local signal='\[\033[38;2;100;10;40m\]'
		local green='\[\033[38;2;190;220;80m\]'
		local slash="${white}${_SLASH}${black}"
		local bg='\[\033[38;2;100;140;160m\]'
		local fg='\[\033[38;2;0;0;0m\]'
		
		local __yellow='\[\033[38;2;210;200;30m\]'
		local __white='\[\033[38;2;255;255;255m\]'
		local __black='\[\033[38;2;0;0;0m\]'
		local __home='\[\033[38;2;200;220;240m\]'
		#local __below_fg='\[\033[38;2;170;210;240m\]'
		#local __below_bg='\[\033[38;2;100;140;160m\]'
		local _home="$HOME"
		local _home_len="${#_home}"

		if [[ "$path" == "~" ]]; then
			#path=" ${__white}${_HOME}${__black} "
			startBG 0 0 0
			path="${__yellow}${_HOME}${__black}"
		elif [[ "${path:0:2}" == "~/" ]]; then
			startBG 100 140 160
			path="${path:2}"
			#path=" ${__white}${_HOME} ${__black}${path//\//${__white}${_SLASH}${__black}}"
			path=" ${__white}${_HOME} ${__black}${path//\//${__white}${_SLASH}${__black}}"
			path="${path}${__black} ${_SLASH// /} "
		elif [[ "$path" == "/" ]]; then
			path="${__yellow}${_ROOT}${__black}"
		elif [[ "${path::1}" == "/" ]]; then
			startBG 100 140 160
			path="${path:1}"
			path=" ${__white}${_ROOT}${__black} ${path//\//${__white}${_SLASH}${__black}}${__black}${_SLASH}"
		else
			startBG 100 140 160
			path=" ${path} "
		fi
		...
		#if [[ "$_pwd" == "/" ]]; then
		#	path="${green}${_SLASH// /}${black}"
		#elif [[ -n "$_home" && "$_pwd" == "$_home" ]]; then
		#	path="${green}${_HOME}${black}"
		#elif [[ -n "$_home" && "${_pwd:0:${_home_len}+1}" == "${_home}/" ]]; then
		#	path="${path:1}"
		#	path="${green}${_HOME}${black}${path//\//${slash}}${_SLASH% }"
		#else
		#	path="${green}/${black}${path:1}"
